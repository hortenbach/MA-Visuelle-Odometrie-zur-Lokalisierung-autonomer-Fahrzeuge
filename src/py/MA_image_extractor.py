...
if __name__ == "__main__":
    ...
    bag_file = (
        f"/home/hortenbach/CrucialM2/rosbags/ORB_set_final/{dataset}/{dataset}_0.db3"
    )
    print(bag_file)
    if os.path.exists(bag_file):
        ...
        camImg_r = [camImg_r[i][1] for i in range(len(camImg_l))]
        rgb_d = parser.get_messages("/carla/ego_vehicle/depth_front/image")
        rgb_d = [rgb_d[i][1] for i in range(len(camImg_l))]
        ...
        for i in range(len(camImg_l)):
            ...
            cv2.imwrite(f"./pictures/{dataset}/left/image_left_{i}.jpg", cv_img_l)
            cv2.imwrite(f"./pictures/{dataset}/right/image_right_{i}.jpg", cv_img_r)
            cv2.imwrite(f"./pictures/{dataset}/depth/image_depth_{i}.jpg", cv_depth)
    else:
